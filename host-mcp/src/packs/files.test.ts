...
test('lists and reads files inside the root', async () => {
  const pack = createFilesPack(defaultConfig({ files: { root, writes: false, maxReadBytes: 1024 } }));
...
test('omits write_file unless writes are enabled', async () => {
  const closed = createFilesPack(defaultConfig({ files: { root, writes: false, maxReadBytes: 1024 } }));
  assert.equal(closed.tools.some((tool) => tool.name === 'write_file'), false);

  const open = createFilesPack(defaultConfig({ files: { root, writes: true, maxReadBytes: 1024 } }));
